...
> Repo này hiện có reference implementation hoàn chỉnh, chạy offline mặc định và tự động
> dùng OpenAI, Tavily, LangGraph, OpenTelemetry khi được cấu hình.
...
Baseline tìm nguồn và thực hiện một lượt tổng hợp.
...
Workflow chạy Supervisor, Researcher, Analyst, Writer và Critic.
...
1. OpenAI LLM client với retry, usage accounting và offline fallback.
2. Tavily search client với local fallback.
3. Supervisor routing.
4. Researcher, Analyst, Writer và Critic.
5. LangGraph workflow với dependency-free fallback runner.
6. JSONL trace export và optional OpenTelemetry spans.
7. Benchmark command và Markdown report.
